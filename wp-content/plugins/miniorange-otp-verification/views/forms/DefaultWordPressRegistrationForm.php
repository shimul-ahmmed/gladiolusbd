...
echo'	<div class="mo_otp_form" id="'.get_mo_class($handler).'"><input type="checkbox" '.esc_attr($disabled).' id="wp_default" class="app_enable" data-toggle="wp_default_options" 
                name="mo_customer_validation_wp_default_enable" value="1"
			'.esc_attr($default_registration).' /><strong>'.esc_attr($form_name).'</strong>';
...
echo'           <br/>
				...
				<div '.(esc_attr($wp_default_type) !== esc_attr($wpreg_phone_type)  ? "hidden" :"").' class="mo_registration_help_desc" 
						...
						<input type="checkbox" '.esc_attr($disabled).' name="mo_customer_validation_wp_reg_restrict_duplicates" value="1"
								'.esc_attr($wp_handle_reg_duplicates).' /><strong>'. mo_( "Do not allow users to use the same phone number for multiple accounts." ).'</strong>
				</div>
				<p>
