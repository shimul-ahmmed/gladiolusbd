...
echo'	<div class="mo_otp_form" id="'.get_mo_class($handler).'"><input type="checkbox" '.esc_attr($disabled).' id="userpro_registration" class="app_enable" data-toggle="userpro_registration_options" name="mo_customer_validation_userpro_registration_enable" value="1"
...
echo'							<div class="mo_registration_help_desc" '.esc_attr($userpro_hidden).' id="userpro_registration_options">
									<p><input type="checkbox" '.esc_attr($disabled).' class="form_options" '.esc_attr($automatic_verification).' id="mo_customer_validation_userpro_verify" name="mo_customer_validation_userpro_verify" value="1"/> &nbsp;<strong>'. mo_("Verify users after registration" ).'</strong><br/></p>
									...
									<div '.(esc_attr($userpro_enabled_type) != esc_attr($userpro_type_phone) ? "hidden" : "").' class="mo_registration_help_desc" id="userpro_registration_phone_instructions" >
											...
											<ol>
												...
												<li><a href="'.esc_url($userpro_field_list).'" target="_blank">'. mo_( "Click Here" ).'</a> '. mo_( "to see your list of UserPro fields." ).'</li>
