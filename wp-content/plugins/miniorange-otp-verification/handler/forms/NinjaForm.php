...
class NinjaForm extends FormHandler implements IFormHandler
{
    ...
    {
        return array_key_exists('option',$_POST) && (strpos(sanitize_text_field($_POST['option']), 'verification_resend_otp')
            || sanitize_text_field($_POST['option'])=='miniorange-validate-otp-form' || sanitize_text_field($_POST['option'])=='miniorange-validate-otp-choice-form');
    }
    ...
    function parseFormDetails()
    {
        ...
        foreach (array_filter($_POST['ninja_form']['form']) as $key => $value) {
            $form[sanitize_text_field($value)]=array('emailkey'=>sanitize_text_field($_POST['ninja_form']['emailkey'][$key]),
                                'phonekey'=>sanitize_text_field($_POST['ninja_form']['phonekey'][$key]));
        }
        return $form;
    }
